...
Tabela pokazuje średni dochód, medianę oraz odchylenie standardowe wynagrodzenia dla dwóch grup pracowników - tych którzy odeszli z firmy i tych którzy w niej zostali. 
Pracownicy, którzy nie odeszli, mają wyższy średni dochód niż ci, którzy odeszli. To sugeruje, że osoby z niższym wynagrodzeniem częściej odchodzą.
Mediana również pokazuje znaczącą różnicę: 5189.5 (pozostali) vs. 3017.5 (odeszli). Oznacza to, że większość osób odchodzących zarabiała znacznie mniej niż większość tych, którzy zostali.
W obu grupach odchylenie standardowe jest wysokie, co oznacza duże zróżnicowanie wynagrodzeń. Jednak pracownicy, którzy pozostali, mają większe zróżnicowanie płac (4775 vs. 3662), co sugeruje, że w tej grupie mogą być zarówno osoby z wysokimi, jak i niskimi zarobkami.
```{r}
...
- 13 - zakres, w którym znajduje się środkowe 50% danych
```{r}
...
Poniżej, obliczono wartość skośności oraz kurtozy, aby określić rozkład danych. Skośność wieku wynosi 0,46 - dodatnia wartość wskazuje na skośność prawostronną oraz więcej młodszych pracowników. Kurtoza wieku wynosi 2,67, co oznacza, że rozkład wieku jest lekko spłaszczony rozkład w porównaniu do normalnego.
```{r}
...
```
### **Testy normlaności**
Test Shapiro-Wilka służy do sprawdzenia normalności rozkładu danej zmiennej. Hipotezowa zerowa mówi, że dane pochodzą z rozkładu zerowego, na rzecz alternatywy, że tak nie jest. Z racji, że wartość p-value jest niższa niż 0.01 odrzucamy hipotezę zerową na rzecz alternatywy. Oznacza to, że rozkład wieku nie jest normalny w obydwóch badanych przypadkach zmiennych.
```{r}
...
- Niska wartość F (0.18) sugeruje, że różnice między średnimi dla działów są bardzo małe w porównaniu do zmienności wewnątrz grup.
Przynależność do działu nie wpływa istotnie na liczbę lat pracy – staż pracowników jest podobny niezależnie od działu.
```{r}
...
- staystyka jest wysoka, co oznacza, różnice między rzeczywistymi a oczekiwanymi wartościami są duże.
...
```
### **Regresja liniowa**
Poniżej sformułowano regresję liniową, w której odejscia z pracy są wyjaśniane przez wiek oraz dochód. Z regresji tej wynika, że:
- każdy dodatkowy rok życia zmniejsza prawdopodobienstwo odejścia z pracy o 0.00465
- z każdym wzrostem miesięcznego wynagrodzenia o 1 zmniejsza prawdopodobienstwo odejścia z pracy o 0.00000736
- zwiększenie dystansu od domu o 1 jednostkę zwiększa prawdopodobienstwo odejścia z pracy o 0.00382
- kazdy dodatkoy rok pracy w firmie zmniejsza prawdopodobienstwo odejścia z pracy o 0.00669
...
gini_coefficient  # Współczynnik Giniego (0 - równość, 1 - nierówność)
```
### **Krzywa Lorenza**
...
```
### **Podsumowanie**
Przeprowadzone analizy wskazują, że osoby o niższych dochodach oraz młodsi pracownicy częściej decydują się na odejście. Dodatkowo, zmienne takie jak odległość od miejsca pracy czy konieczność pracy w nadgodzinach również wpływają na rotację pracowników. Wyniki te mogą być cenną wskazówką dla pracodawców w kontekście strategii retencji i polityki wynagrodzeń.


# **Etap 4. Wnioskowanie statystyczne**

# ==================================================

# a) Test t-Studenta: Czy wiek różni się istotnie między osobami, które odeszły i zostały?

t_test_age \<- t.test(Age \~ Attrition, data = tabela_knn)
print(t_test_age)

# b) Test t-Studenta: Czy dochód różni się istotnie między grupami Attrition?

t_test_income \<- t.test(MonthlyIncome \~ Attrition, data = tabela_knn)
print(t_test_income)

# c) Test chi-kwadrat: Czy odejścia zależą od działu?

chi_test_department \<-
chisq.test(table(tabela_knn$Attrition, tabela_knn$Department))
print(chi_test_department)

# d) Test chi-kwadrat: Czy odejścia zależą od stanowiska?

chi_test_jobrole \<-
chisq.test(table(tabela_knn$Attrition, tabela_knn$JobRole))
print(chi_test_jobrole)

# ==================================================

# WIZUALIZACJE Z ggstatsplot

# ==================================================

# a) Różnice wieku między osobami, które odeszły i zostały
...
# b) Porównanie miesięcznych dochodów między osobami, które odeszły i zostały
...
# c) Zależność odejść od działu

ggstatsplot::ggbarstats( data = tabela_knn, x = Department, y =
Attrition, title = "Zależność odejść od działu", xlab = "Dział", ylab =
"Proporcja odejść", messages = FALSE )

# d) Zależność odejść od stanowiska

ggstatsplot::ggbarstats( data = tabela_knn, x = JobRole, y = Attrition,
title = "Zależność odejść od stanowiska", xlab = "Stanowisko", ylab =
"Proporcja odejść", messages = FALSE )

# e) Zależność między wiekiem a latami pracy w kontekście odejść

ggstatsplot::ggscatterstats( data = tabela_knn, x = Age, y =
TotalWorkingYears, title = "Zależność między wiekiem a latami pracy w
kontekście odejść", xlab = "Wiek", ylab = "Lata pracy", color =
tabela_knn\$Attrition, messages = FALSE )
