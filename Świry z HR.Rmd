...
Dla potwierdzenia wcześniejszych analizch, zastosowano funkcję, która zwraca nazwy wierszy zawierające braki danych. Tak jak wyszło to we wcześniejszych metodach, zmiennimi z brakami są Age, Attrition oraz MonthlyIncome.
...
Kolejną metodą sprawdzającą braki danych jest shadowmapa. Pokazuje ona, w których kolumnach znajdują się
brakujące dane oraz zwraca ich liczbę.
...
Poniżej zastosowano bardziej szczegółowy wykres, który pokazuje nie tylko w których zmiennych są braki, ale także wskazuje wiersz, w ktorym one występują.
...
library(datarium) library(ggplot2)
