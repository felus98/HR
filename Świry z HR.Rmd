...
Policzono podstawowe statystyki opisowe takie jak mediana, moda,
odchylenie stadardowe, minimum, maksimum itd. dla trzech wybranych
zmiennych (wiek, średnie wynagordzenie dzienne, dystans z domu do
pracy). Wyniki zaprezentowano w tabeli poniżej. Przykładowo dla wieku:
średnia wynosi 36,88 lat, połowa pracowników ma mniej, a połowa więcej
niż 35 lat, czy rozpiętość wieku wynosi 18-60 lat.

```{r}
describe_tabela <- describe(tabela_knn[, c("Age", "DailyRate", "DistanceFromHome")])
print(describe_tabela)
```
...
```
### **Regresja liniowa**
Poniżej sformułowano regresję liniową, w której odejscia z pracy są wyjaśniane przez wiek oraz dochód. Z regresji tej wynika, że każdy dodatkowy rok życia zwię
```{r}
reg_model <- lm(Attrition ~ Age+MonthlyIncome, data = tabela_knn)
summary(reg_model)
```
...
```{r}
ggplot(tabela_knn, aes(x = Age, y = TotalWorkingYears)) +
  geom_point() +
  geom_smooth(method = "lm", col = "red") +
  ggtitle("Regresja liniowa: Wiek vs Lata pracy") +
  xlab("Wiek") +
  ylab("Lata pracy")
```

# 7. NIERÓWNOŚCI DOCHODOWE (WSPÓŁCZYNNIK GINIEGO)
...
attrition_summary \<- tabela_knn %\>% group_by(Attrition) %\>%
summarise(Liczba = n(), Procent = round(n() / nrow(tabela_knn) \* 100,
2))
...
age_summary \<- tabela_knn %\>% group_by(Attrition) %\>%
summarise(Średni_wiek = round(mean(Age, na.rm = TRUE), 0), Mediana_wieku
= median(Age, na.rm = TRUE), Odchylenie_standardowe = round(sd(Age,
na.rm = TRUE), 1))

print(age_summary)

# Podstawowe statystyki dochodów w zależności od odejścia
