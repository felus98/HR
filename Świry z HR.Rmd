...
variance <- round(var(tabela_knn$Age), 2)  # Wariancja (zaokrąglona)
std_dev <- round(sd(tabela_knn$Age), 2)  # Odchylenie standardowe (zaokrąglone)
cv_age <- round((std_dev / mean(tabela_knn$Age)) * 100, 2)  # Współczynnik zmienności (%)

# Skośność i kurtoza (zaokrąglone)
...
# Przedziały zgodnie z regułą trzech sigm
...
# Tworzenie tabeli (wszystkie statystyki w jednej)
...
odchylenie standardowe. Wyniki są następujące:
...
wynosi 2,67, co oznacza, że rozkład wieku jest lekko spłaszczony, w
...
zmiennej. Hipoteza zerowa mówi, że dane pochodzą z rozkładu normalnego,
względem alternatywy, że tak nie jest. Z racji, że wartość p-value jest
niższa niż 0.01 odrzucamy hipotezę zerową na rzecz alternatywy. Oznacza
to, że rozkład wieku nie jest normalny w obydwu badanych przypadkach
zmiennych.
...
# Test t-studenta (dla Age względem Attrition)
...
# Korelacja między Age a TotalWorkingYears
correlation_test <- cor.test(tabela_knn$Age, tabela_knn$TotalWorkingYears)

# Tworzenie tabeli podsumowującej wyniki testów statystycznych
data_frame <- data.frame(
...
Wniosek: Odejścia są silnie zależne od stanowiska. Niektóre stanowiska
mogą mieć znacznie większą rotację niż inne.
