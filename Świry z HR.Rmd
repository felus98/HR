...
install.packages("moments")  # Dla skośności i kurtozy
install.packages("ineq")  # Dla współczynnika Giniego i krzywej Lorenza
install.packages("ggplot2")  # Dla wizualizacji
install.packages("corrplot")  # Wizualizacja korelacji
install.packages("psych")  # Dla statystyk opisowych
install.packages("MASS")  # Dla testu normalności
...
# Wczytanie danych (jeśli jeszcze nie są wczytane)
# tabela_knn <- read.csv("Twoje_dane.csv")
...
describe(tabela_knn)
```
# Pełniejsza analiza dla danych numerycznych

# Miary tendencji centralnej
```{r}
mean(tabela_knn$Age)  # Średnia arytmetyczna
median(tabela_knn$Age)  # Mediana
table(tabela_knn$Age)[which.max(table(tabela_knn$Age))]  # Moda
```

# Miary pozycyjne
```{r}
quantile(tabela_knn$Age, probs = c(0.25, 0.5, 0.75))  # Kwartyle
IQR(tabela_knn$Age)  # Rozstęp międzykwartylowy
```

# Miary zróżnicowania
```{r}
range(tabela_knn$Age)  # Rozstęp
var(tabela_knn$Age)  # Wariancja
sd(tabela_knn$Age)  # Odchylenie standardowe
...
```{r}
skewness(tabela_knn$Age)  # Skośność (>0 prawostronna, <0 lewostronna)
kurtosis(tabela_knn$Age)  # Kurtoza (>3 leptokurtyczny, <3 platokurtyczny)
```
...
# Test t-Studenta: czy średni wiek różni się między osobami, które odeszły a tymi, które zostały
```{r}
t.test(Age ~ Attrition, data = tabela_knn)
```

# ANOVA: czy lata pracy różnią się istotnie między działami
```{r}
anova_model <- aov(TotalWorkingYears ~ Department, data = tabela_knn)
summary(anova_model)
```

# Test chi-kwadrat: czy rezygnacja zależy od działu
```{r}
...
# Macierz korelacji
```{r}
cor_matrix <- cor(tabela_knn[, sapply(tabela_knn, is.numeric)], use = "complete.obs")
...
# 6. REGRESJA LINIOWA
```{r}
reg_model <- lm(TotalWorkingYears ~ Age, data = tabela_knn)
summary(reg_model)
```
...
attrition_summary <- tabela_knn %>%
  group_by(Attrition) %>%
  summarise(Liczba = n(), Procent = round(n() / nrow(tabela_knn) * 100, 2))

print(attrition_summary)

# Podstawowe statystyki wieku w zależności od odejścia
age_summary <- tabela_knn %>%
  group_by(Attrition) %>%
  summarise(Średni_wiek = round(mean(Age, na.rm = TRUE), 0),
            Mediana_wieku = median(Age, na.rm = TRUE),
            Odchylenie_standardowe = round(sd(Age, na.rm = TRUE), 1))

print(age_summary)

# Podstawowe statystyki dochodów w zależności od odejścia
income_summary <- tabela_knn %>%
  group_by(Attrition) %>%
...
chi_test_department <- chisq.test(table(tabela_knn$Attrition, tabela_knn$Department))
print(chi_test_department)

# d) Test chi-kwadrat: Czy odejścia zależą od stanowiska?
chi_test_jobrole <- chisq.test(table(tabela_knn$Attrition, tabela_knn$JobRole))
...
ggstatsplot::ggbetweenstats(
  ...
  x = Attrition,
  y = Age,
  title = "Porównanie wieku między osobami, które odeszły i zostały",
  xlab = "Attrition (Czy pracownik odszedł?)",
  ylab = "Wiek",
  messages = FALSE
)
...
ggstatsplot::ggbetweenstats(
  ...
  x = Attrition,
  y = MonthlyIncome,
  title = "Porównanie miesięcznych dochodów między osobami, które odeszły i zostały",
  xlab = "Attrition (Czy pracownik odszedł?)",
  ylab = "Miesięczny dochód",
  messages = FALSE
)
...
ggstatsplot::ggbarstats(
  ...
  x = Department,
  y = Attrition,
  title = "Zależność odejść od działu",
  xlab = "Dział",
  ylab = "Proporcja odejść",
  messages = FALSE
)
...
ggstatsplot::ggbarstats(
  ...
  x = JobRole,
  y = Attrition,
  title = "Zależność odejść od stanowiska",
  xlab = "Stanowisko",
  ylab = "Proporcja odejść",
  messages = FALSE
)
...
ggstatsplot::ggscatterstats(
  ...
  x = Age,
  y = TotalWorkingYears,
  title = "Zależność między wiekiem a latami pracy w kontekście odejść",
  xlab = "Wiek",
  ylab = "Lata pracy",
  color = tabela_knn$Attrition,
  messages = FALSE
)
