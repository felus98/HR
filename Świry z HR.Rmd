...
#zmienić na boxplot - gotowe, poprawic inteprretacje
```{r}
...
```
# Wykres 12 - Zależność liczby lat od ostatniego awansu względem odejść
...
ggplot(data = tabela_knn, aes(x = YearsSinceLastPromotion, fill = Attrition)) +
  ...
  theme_minimal()
```
# Heatmapa - Heatmapa korelacji zmiennych z odejściami pracowników

``` {r}
# Konwersja zmiennej 'Attrition' do wartości binarnych (1 = "Yes", 0 = "No")
...
# Tworzenie heatmapy przy użyciu ggplot2
melted_cor_matrix <- melt(cor_matrix)

# Generowanie heatmapy
ggplot(data = melted_cor_matrix, aes(x = Var1, y = Var2, fill = value)) + 
  geom_tile(color = "white") +
  scale_fill_gradient2(low = "blue", high = "red", mid = "snow2", midpoint = 0, limit = c(-1,1), space = "Lab", name="Korelacja") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  coord_fixed() +
  labs(title = "Heatmapa korelacji wybranych zmiennych", x = "", y = "")
```
...
# 2. Rozkład odejść w zależności od wieku (Age)
ggplot(tabela_knn, aes(x = Age, fill = Attrition)) +
  geom_density(alpha = 0.5) +
  labs(title = "Rozkład odejść w zależności od wieku", x = "Wiek", y = "Gęstość") +
  scale_fill_manual(values = c("No" = "lightblue", "Yes" = "darkred"), name = "Attrition") +
  theme_minimal()
...
# 4. Rozkład odejść według działów (Department)
ggplot(tabela_knn, aes(x = Department, fill = Attrition)) +
  geom_bar(position = "fill") +
  labs(title = "Rozkład odejść w zależności od działu", y = "Proporcja", x = "Dział") +
  scale_fill_manual(values = c("No" = "lightblue", "Yes" = "darkred"), name = "Attrition") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# 6. Heatmapa odejść w zależności od lat pracy i wynagrodzenia
ggplot(tabela_knn, aes(x = YearsAtCompany, y = MonthlyIncome, fill = Attrition)) +
  geom_tile() +
  labs(title = "Odejścia w zależności od lat pracy i wynagrodzenia", x = "Lata pracy", y = "Miesięczne wynagrodzenie") +
  scale_fill_manual(values = c("No" = "skyblue3", "Yes" = "red"), name = "Attrition") +
  theme_minimal()
...
#te wykresy juz mamy - mysle, ze az tyle nie potrzeba bez zmiennej attrition
...
### Wykres 11 - Korelacja rotacji pomiedzy dzialami(Department) a odejscia z pracy (Atrittion)
# Wykres 12 - Zależność liczby lat od ostatniego awansu względem odejść
# Heatmapa korelacji zmiennych z odejściami pracowników
# Heatmapa - wersja 2 korelacji zmiennych z odejściami pracowników
# Rozklad - odejscia wgledem plci (Gender), wieku (Age), i stanu cywilnego (MaritialStatus)
# 1. Rozkład odejść w zależności od płci (Gender)
# 2. Rozkład odejść w zależności od wieku (Age)
# 3. Rozkład odejść w zależności od stanu cywilnego (MaritalStatus)
...
# zrobic?
# Rozkład odejść w zależności od demografii – przegląd odejść z podziałem na płeć, wiek, itp.
# Analiza nadgodzin a odejścia – sprawdzenie, jak nadgodziny wpływają na decyzję o odejściu.
# Wpływ satysfakcji z pracy na odejścia – przeanalizowanie, jak poziom satysfakcji z pracy wpływa na odejścia.
