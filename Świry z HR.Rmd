...
```{r}
set.seed(42)
tabela_knn <- data.frame(Age = rnorm(300, mean = 35, sd = 10))
 
quantiles <- quantile(tabela_knn$Age, probs = c(0.25, 0.5, 0.75))
mean_age <- mean(tabela_knn$Age)
 
ggplot(tabela_knn, aes(x = "", y = Age)) +
  geom_boxplot(fill = "lightblue", color = "black", outlier.color = "red", outlier.shape = 16, outlier.size = 3, linewidth = 1) +
